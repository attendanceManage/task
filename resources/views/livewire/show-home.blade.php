    ...
    <div class="row align-items-center justify-content-center">
      <div class="col-lg-6 mb-5 mb-lg-0">
        <div class="block text-center text-lg-start pe-0 pe-xl-5">
          <h1 class="text-capitalize mb-4">{{$slider->name}}</h1>
          <p class="mb-4">{{$slider->name}}</p> <a type="button"
       ...
       @foreach($products as $key=>$value)
			...
			<div class="icon-box-item text-center col-lg-4 col-md-6 mb-4">
				<div class="rounded shadow py-5 px-4">
					<div> 
          <img src="{{ config('app.url')}}/storage/{{$value->image}}" width="200"  height="200">  
					</div>
					<h3 class="mb-3">{{$value->name}}</h3>
					<p class="mb-4">{{strip_tags($value->description)}}</p> <a class="btn btn-sm btn-outline-primary" href="service-details.html">View Details <i class="las la-arrow-right ms-1"></i></a>
				</div>
			</div>
      @endforeach
     @endif
...
<section class="section">
  <div class="container">
    ...
    @if(!empty($frequentlyQuestions))
   
      @foreach($frequentlyQuestions as $key=>$value)
      ...
      <div class="col-lg-4 col-md-6 service-item">
        <a class="text-black" href="service-details.html">
          <div class="block"> 
            ...
            <h3 class="mb-3 service-title">{{$value->name}}</h3>
            
            <blockquote class="text-sl italic font-semibold text-gray-400 dark:text-white">
              {{strip_tags($value->description)}}
             </blockquote>
